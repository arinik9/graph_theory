...
modularity_directed=function(g, community_str,weighted){
    if (weighted){
	  ...
	  sums=0
	  m=as.numeric(sum(E(g)$weight))

    for ( i in coms){
	      my_nodes=which(community_str==i)
	      inside_tot=as.numeric(sum(E(induced.subgraph(g,my_nodes))$weight))
	      outer_tot=as.numeric(sum(E(g)[from(my_nodes)]$weight))
	      inner_tot=as.numeric(sum(E(g)[to(my_nodes)]$weight))
	      eii=(inside_tot)/(m)
